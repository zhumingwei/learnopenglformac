...
class Shader
{
    public:
        ...
        void init(const char* vertexPath, const char* fragmentPath, const char* geometryPath = ""){
            ...
            bool hasGeometry = strncmp(geometryPath, "",20);
        try
        {
        ...
        glLinkProgram(ID);
        checkCompileErrors(ID, "PROGRAM","p");
        ...
        private : void
                  checkCompileErrors(unsigned int shader, std::string type, const char *path)
        {
            int success;
            char infoLog[1024];
            if (type != "PROGRAM")
